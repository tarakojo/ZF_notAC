...
next 
  ...
  have H : "Pn_auto(converse(\<pi>)) ` (Pn_auto(\<pi>)`x) \<in> HS" 
    ...
    by auto
  have "Pn_auto(converse(\<pi>)) ` (Pn_auto(\<pi>)`x) = (Pn_auto(converse(\<pi>)) O Pn_auto(\<pi>)) ` x" 
    apply(rule eq_flip, rule comp_fun_apply)
    apply(rule Pn_auto_type)
    using assms \<G>_P_auto_group is_P_auto_group_def
    by auto
  also have "... = Pn_auto(converse(\<pi>) O \<pi>) ` x" 
    apply(subst Pn_auto_comp)
    apply(rule P_auto_converse)
    using assms \<G>_P_auto_group is_P_auto_group_def
    by auto
  also have "... = Pn_auto(id(P)) ` x" 
    ...
    using assms \<G>_P_auto_group is_P_auto_group_def is_P_auto_def 
    by auto
