...
proof - 
  ...
  proof - 
    ...
    have "forces_mem(p, x, y) \<longleftrightarrow>  dense_below({ q\<in>P. \<exists>s. \<exists>r. r\<in>P \<and> \<langle>s,r\<rangle> \<in> y \<and> q\<preceq>r \<and> forces_eq(q,x,s)}, p)"
      using forces_mem_iff_dense_below pinP apply auto done 
    ...
    proof - 
      ...
      have "\<pi>``X = Y" 
        ...
        apply (rule_tac A=X and r=\<pi> and b=x in imageE) apply simp 
        apply (simp add:Y_def; rule conjI)
        using piauto bij_is_fun unfolding is_P_auto_def Pi_def apply blast 
      proof - 
        ...
        have H: "q \<in> P \<and> \<pi>`q = piq" 
          ...
          using bij_is_fun piauto unfolding is_P_auto_def assms apply blast using assms by simp 
        then have H2:"\<pi>`q = piq" by auto 
        then have H3:"\<pi>`q \<preceq> \<pi> ` r"
          apply (rule_tac P_auto_preserves_leq)
          using piauto H srH by auto 
        show "\<exists>s r. r \<in> P \<and> \<langle>s, r\<rangle> \<in> Pn_auto(\<pi>) ` y \<and> piq \<preceq> r \<and> forces_eq(piq, Pn_auto(\<pi>)`x, s)"
          ...
          apply (auto) 
          using P_auto_value piauto srH apply simp 
          apply (rule_tac a="{<Pn_auto(\<pi>)`z, \<pi>`v>. <z, v> \<in> y}" and b="Pn_auto(\<pi>) ` y" in ssubst)
          using Pn_auto ypname apply simp 
          apply (rule_tac pair_relI) using srH H H2 H3 apply simp_all
          apply (rule_tac a="\<pi>`q" and b = "piq" in ssubst) 
          using H apply simp apply (rule_tac P="EQ(q, x, s)" in mp) 
          apply (simp add: EQ_def) 
          apply (cases "P_rank(s) \<le> b") 
          apply (rule_tac b=b in EQH) using bina xpname ypname H xrank apply simp_all using ypname srH P_name_domain_P_name apply simp 
          apply (rule_tac P="b \<le> P_rank(s)" in mp) apply (rule impI) 
          apply (rule_tac b="P_rank(s)" in EQH) 
          apply (rule_tac ltD) apply(rule_tac b="P_rank(y)" in lt_le_lt) using ypname domain_P_rank_lt yrank P_name_domain_P_name P_rank_ord apply simp_all   
          apply (rule_tac j=b in le_trans) using xrank apply simp_all apply(rule_tac P="b < P_rank(s)" in mp) using lt_succ_lt apply simp 
          using not_le_iff_lt orda Ord_in_Ord P_rank_ord apply auto done
      next 
        ...
        have "\<exists>s r. <s, r> \<in> y \<and>  <s', r'> = <Pn_auto(\<pi>)`s, \<pi>`r>" 
          apply (rule_tac pair_rel_arg) using relation_P_name ypname s'r'H Pn_auto by auto
        ...
        have rinP : "r \<in> P" 
          ...
          using ypname srH qH apply simp_all done 
        ...
        then show "q' \<in> \<pi>``X" 
          apply (rule_tac a=q in imageI) apply (rule_tac a="\<pi>`q" and b=q' in ssubst)
          using qH apply simp apply (rule_tac function_apply_Pair) 
          using P_auto_is_function P_auto_domain qH piauto apply simp_all
          unfolding X_def apply simp 
          apply (rule_tac x=s in exI)
          apply (rule_tac x=r in exI)
          apply (rule conjI) using rinP apply simp 
          apply (rule conjI) using srH qH apply simp_all 
          apply (rule conjI) using qr apply simp 
          apply (rule_tac P="EQ(q, x, s)" in mp) 
          using fH apply (simp add: EQ_def) 
          apply (cases "P_rank(s) \<le> b") 
          apply (rule_tac b=b in EQH) using bina xpname ypname qH xrank apply simp_all using ypname srH P_name_domain_P_name apply simp 
          apply (rule_tac P="b \<le> P_rank(s)" in mp) apply (rule impI) 
          apply (rule_tac b="P_rank(s)" in EQH) 
          apply (rule_tac ltD) apply(rule_tac b="P_rank(y)" in lt_le_lt) using ypname domain_P_rank_lt yrank P_name_domain_P_name P_rank_ord apply simp_all   
          apply (rule_tac j=b in le_trans) using xrank apply simp_all apply(rule_tac P="b < P_rank(s)" in mp) using lt_succ_lt apply simp 
      ...
      also have "... \<longleftrightarrow> forces_mem(\<pi>`p, Pn_auto(\<pi>)`x, Pn_auto(\<pi>)`y)" 
        ...
        using P_auto_value pinP piauto by auto 
    ...
    proof - 
      ...
      have srank_lemma : "\<And>s. s \<in> domain(x) \<union> domain(y) \<Longrightarrow> P_rank(s) < a" 
          ...
          apply (rule_tac P="\<exists>p. <s, p> \<in> x" in mp) apply clarify using xpname domain_P_rank_lt apply simp 
          apply (simp add:domain_def) apply blast using xrank apply simp 
          apply (rule_tac b="P_rank(y)" in lt_le_lt) 
          apply (rule_tac P="\<exists>p. <s, p> \<in> y" in mp) apply clarify using ypname domain_P_rank_lt apply simp 
          apply (simp add:domain_def) apply blast using yrank apply simp done 
      ...
      proof - 
        ...
        then have qlep : "q \<preceq> p"   
          ...
          using piauto pinP q'le by auto 

        have s'pname : "s' \<in> P_names" 
          apply (rule_tac A="domain(Pn_auto(\<pi>)`x)" and B="domain(Pn_auto(\<pi>)`y)" and c=s' in UnE)
          using s'in apply simp apply (rule_tac x="(Pn_auto(\<pi>) ` x)" in P_name_domain_P_name') 
          using xpname Pn_auto_value piauto apply simp_all
          apply (rule_tac x="(Pn_auto(\<pi>) ` y)" in P_name_domain_P_name') 
          using ypname Pn_auto_value piauto apply simp_all done 
        ...
        proof - 
          ...
          then have "\<exists>t p. <t, p> \<in> x \<and> <s', p'> = <Pn_auto(\<pi>)`t, \<pi>`p>" 
            apply (rule_tac pair_rel_arg) using xpname relation_P_name apply simp_all done 
          ...
          then have "s = t" 
            ...
            using sH apply simp apply (rule_tac function_apply_Pair) 
            using piauto Pn_auto_function apply simp 
            using sH Pn_auto_domain apply simp 
            apply (rule_tac a="Pn_auto(\<pi>)`t" and b=s' in ssubst) 
            using tpH apply simp apply (rule_tac function_apply_Pair) 
            using piauto Pn_auto_function apply simp 
            using sH Pn_auto_domain apply simp 
            using Pn_auto_bij piauto bij_is_inj apply auto using tpH xpname P_name_domain_P_name by simp 
        ...
        also have "... \<longleftrightarrow> forces_mem(q, s, x)" 
          ...
          apply (simp add:MEM_def) apply(rule_tac b="P_rank(s)" in MEMH) 
          using srank ltD sH xpname qH le_refl xrank P_rank_ord by auto        
        ...
        also have "... \<longleftrightarrow> forces_mem(\<pi>`q, Pn_auto(\<pi>) ` s, Pn_auto(\<pi>) ` y)" 
          ...
          apply (simp add:MEM_def) apply(rule_tac b="P_rank(s)" in MEMH) 
          using srank ltD sH ypname qH le_refl yrank P_rank_ord by auto  
      ...
      next
        ...
        proof - 
          ...
          then have "<Pn_auto(\<pi>)`s, \<pi>`p> \<in> Pn_auto(\<pi>)`x" 
            using assms piauto Pn_auto by auto 
        ...
        have s'in : "Pn_auto(\<pi>)`s \<in> domain(Pn_auto(\<pi>)`x) \<union> domain(Pn_auto(\<pi>) ` y)" 
          apply (rule_tac A="domain(x)" and B="domain(y)" and c=s in UnE)  
          using sin apply simp 
          apply (rule_tac UnI1) apply (rule_tac domain_lemma) using xpname apply simp apply simp 
          apply (rule_tac UnI2) apply (rule_tac domain_lemma) using ypname apply simp_all done 
        ...
        have "forces_mem(q, s, x) \<longleftrightarrow> forces_mem(\<pi>`q, Pn_auto(\<pi>)`s, Pn_auto(\<pi>) ` x)" 
          ...
          apply (rule_tac b="P_rank(s)" in MEMH)
          using srank ltD apply simp using P_name_domain_P_name' sin xpname ypname apply blast 
          using xpname qinP le_refl xrank P_rank_ord apply simp_all done 
        also have "... \<longleftrightarrow> forces_mem(\<pi>`q, Pn_auto(\<pi>)`s, Pn_auto(\<pi>) ` y)" 
        ...
        also have "... \<longleftrightarrow> forces_mem(q, s, y)" 
          ...
          apply (rule_tac b="P_rank(s)" in MEMH)
          using srank ltD apply simp using P_name_domain_P_name' sin xpname ypname apply blast 
          using ypname qinP le_refl yrank P_rank_ord apply simp_all done 
      ...
      also have "... \<longleftrightarrow> forces_eq(\<pi>`p, Pn_auto(\<pi>)`x, Pn_auto(\<pi>)`y)" 
        apply(rule iff_flip)
        apply(rule_tac def_forces_eq) using pinP piauto P_auto_value by auto 
      finally show "EQ(p,x,y)"
        unfolding EQ_def by auto 
    qed

    have main : "\<And>a. Ord(a) \<longrightarrow> Q(MEM, a, a) \<and> Q(EQ, a, a)" 
      apply (rule_tac eps_induct) apply (rule impI)
    proof - 
      fix a assume ih: "\<forall>b \<in> a. Ord(b) \<longrightarrow> Q(MEM, b,b) \<and> Q(EQ, b,b)" 
      and orda : "Ord(a)" 
      then have "\<forall>b \<in> a. Q(MEM, b, a)" 
        apply (rule_tac MEM_step) apply simp 
        using orda Ord_in_Ord by auto 
      then have H : "Q(EQ, a,a)" apply (rule_tac EQ_step) using orda apply auto done 
      then have "\<forall>b \<in> succ(a). Q(EQ, b, b)" 
        apply clarify 
        apply (rule_tac i=b and j=a in leE) using ltI orda apply simp using ltD ih orda lt_Ord apply auto done 
      then have "\<forall>b \<in> succ(a). Q(MEM, b, succ(a))"
        apply (rule_tac MEM_step) using orda apply auto done 
      then have "Q(MEM, a, succ(a))" by auto 
      then have "Q(MEM, a, a)" 
        unfolding Q_def apply clarify 
        apply (rule_tac P="P_rank(y) \<le> succ(a)" in mp) apply auto 
        apply (rule_tac j=a in le_trans) 
        apply simp apply (rule_tac j="succ(a)" in lt_trans) using le_refl orda by auto 
    ...
    have rle: "P_rank(x) \<le> r \<and> P_rank(y) \<le> r" 
      ...
      apply simp using P_rank_ord apply simp apply (simp add:r_def) 
      apply (rule_tac P="r = P_rank(x)" in mp) 
      using P_rank_ord apply simp 
      apply (rule_tac P="P_rank(y) < P_rank(x)" in mp) 
      using lt_succ_lt apply simp 
      using not_le_iff_lt P_rank_ord apply simp 
      apply (simp add:r_def) done 
...
lemma symmetry_lemma':
  ...
  apply (rule impI) using envin phiin phiarity pin apply blast  
  using phiin 
proof(induct)
  case (Member x y)
  then show ?case 
    ...
    apply(rule_tac forces_P_namify) apply simp apply simp apply simp apply simp
    apply (rule_tac P="x < length(env)" in mp) apply (rule_tac P="y < length(env)" in mp)
    apply clarify 
    ...
    apply(rule_tac Forces_Member) apply simp
    using P_names_in_M apply simp using P_names_in_M apply simp apply(rule_tac A=M in map_type) apply simp 
    using P_namify_value_in_M apply simp apply simp apply simp apply simp apply simp 
    apply(rule_tac Q="forces_mem(\<pi>`p, Pn_auto(\<pi>)`nth(x, map(P_namify, env)), Pn_auto(\<pi>)`nth(y, map(P_namify, env)))" in iff_trans) 
    apply(rule_tac symmetry_lemma_mem) using piauto apply simp apply simp apply simp apply simp 
    apply(rule iff_flip) apply(rule_tac Forces_Member) using P_auto_value piauto apply simp 
    using piauto Pn_auto_value_in_M apply simp using piauto Pn_auto_value_in_M apply simp 
    apply(rule_tac A=M in map_type) apply(rule_tac A=M in map_type) apply simp using P_namify_value_in_M apply simp 
    apply(rule_tac P="xa \<in> P_names" and Q="xa \<notin> P_names" in disjE) apply simp 
    using P_names_in_M Pn_auto_value_in_M piauto apply simp apply(rule_tac a="0" and b="Pn_auto(\<pi>)`xa" in ssubst) 
    apply(rule_tac apply_0) using Pn_auto_domain apply simp using zero_in_M apply simp 
    using nth_map apply simp using nth_map apply simp apply simp apply simp_all
    apply(rule_tac P_namify_value_in_P_names) apply(rule_tac nth_type) apply simp apply simp 
    apply(rule_tac P_namify_value_in_P_names) apply(rule_tac nth_type) apply simp apply simp 
    apply (rule_tac b="succ(x) \<union> succ(y)" in lt_le_lt) using ltD ltI apply simp_all
    apply (rule_tac b="succ(x) \<union> succ(y)" in lt_le_lt) using ltD ltI apply simp_all done
next
  case (Equal x y)
  then show ?case
    apply (clarify) 
    ...
    apply (rule_tac Forces_Equal) apply simp 
    using P_names_in_M apply simp using P_names_in_M apply simp apply(rule_tac A=M in map_type) apply simp 
    using P_namify_value_in_M apply simp apply simp apply simp apply simp apply simp 
    apply(rule_tac Q="forces_eq(\<pi>`p, Pn_auto(\<pi>)`nth(x, map(P_namify, env)), Pn_auto(\<pi>)`nth(y, map(P_namify, env)))" in iff_trans) 
    apply(rule_tac symmetry_lemma_eq) using piauto apply simp apply simp apply simp apply simp 
    apply(rule iff_flip) apply(rule_tac Forces_Equal) using P_auto_value piauto apply simp 
    using piauto Pn_auto_value_in_M apply simp using piauto Pn_auto_value_in_M apply simp 
    apply(rule_tac A=M in map_type) apply(rule_tac A=M in map_type) apply simp using P_namify_value_in_M apply simp 
    apply(rule_tac P="xa \<in> P_names" and Q="xa \<notin> P_names" in disjE) apply simp 
    using P_names_in_M Pn_auto_value_in_M piauto apply simp apply(rule_tac a="0" and b="Pn_auto(\<pi>)`xa" in ssubst) 
...
next
  ...
  have ex_lemma2 : "\<And>A. (\<exists>q \<in> M. q \<in> P \<and> A(q)) \<longleftrightarrow> (\<exists>q \<in> M. q \<in> P \<and> A(\<pi>`q))"
    ...
    using transM P_in_M apply auto 
    using piauto unfolding is_P_auto_def using bij_is_surj unfolding surj_def apply auto 
    apply (rule_tac P="\<pi>`q \<in> P" in mp) apply auto using P_auto_value piauto by auto 
  ...
  then show ?case
    ...
    apply (rule_tac Forces_Nand) using pin apply simp apply simp apply simp apply simp
    ...
    apply (rule not_cong) apply(rule_tac ex_lemma) 
    using piauto P_auto_preserves_leq P_auto_preserves_leq' apply blast 
    using piauto P_auto_preserves_leq P_auto_preserves_leq' apply blast 
    ...
    apply (rule not_cong) apply(rule iff_flip) apply(rule_tac ex_lemma2) 
    apply(rule iff_flip) apply (rule_tac Forces_Nand)
    apply (rule_tac P_auto_value) using piauto apply simp_all 
    apply(rule_tac A=M in map_type) apply(rule_tac A=M in map_type) apply simp using P_namify_value_in_M apply simp
    apply(rule_tac P="x \<in> P_names" and Q="x \<notin> P_names" in disjE) apply simp 
    using P_names_in_M Pn_auto_value_in_M piauto apply simp apply(rule_tac a="0" and b="Pn_auto(\<pi>)`x" in ssubst) 
    apply(rule_tac apply_0) using Pn_auto_domain apply simp using zero_in_M apply simp 
    apply (cases "arity(\<phi>) \<le> arity(\<psi>)")  
    apply (rule_tac b="arity(\<psi>)" and a="arity(\<phi>) \<union> arity(\<psi>)" in ssubst) 
    apply (rule eq_flip) apply (rule_tac nat_union_abs1) using arity_type Ord_nat apply simp_all
    apply (rule_tac P="arity(\<psi>) < arity(\<phi>)" in mp) apply (rule impI)
    apply (rule_tac j="arity(\<phi>) \<union> arity(\<psi>)" in lt_trans) using ltD ltI apply simp_all
    using not_le_iff_lt arity_type Ord_nat apply simp 
    apply (cases "arity(\<psi>) \<le> arity(\<phi>)")  
    apply (rule_tac b="arity(\<phi>)" and a="arity(\<phi>) \<union> arity(\<psi>)" in ssubst) 
    apply (rule eq_flip) apply (rule_tac nat_union_abs2) using arity_type Ord_nat apply simp_all
    apply (rule_tac P="arity(\<phi>) < arity(\<psi>)" in mp) apply (rule impI)
    apply (rule_tac j="arity(\<phi>) \<union> arity(\<psi>)" in lt_trans) using ltD ltI apply simp_all
    using not_le_iff_lt arity_type Ord_nat apply simp done 
next
  ...
  have iff_lemma : "\<And>Q. (\<forall>x \<in> M. Q(P_namify(x))) \<longleftrightarrow> (\<forall>x \<in> M. Q(Pn_auto(\<pi>)`P_namify(x)))" 
    apply (rule iffI) apply auto 
  proof - 
    fix Q x assume assms : "\<forall>x\<in>M. Q(P_namify(x))" "x \<in> M" 
    have H:"Pn_auto(\<pi>) ` P_namify(x) \<in> P_names" 
      using assms P_namify_value_in_P_names Pn_auto_value piauto apply auto done 
    then have "Q(P_namify(Pn_auto(\<pi>) ` P_namify(x)))" using P_names_in_M assms by auto 
  ...
  next 
    ...
    then have "Q(Pn_auto(\<pi>) ` P_namify(y))" 
      using P_names_in_M assms by auto 
    ...
    show "map(P_namify, map(\<lambda>x. Pn_auto(\<pi>) ` x, map(P_namify, l))) = map(\<lambda>x. Pn_auto(\<pi>) ` x, map(P_namify, l))" 
      ...
      next
        ...
        then show ?case apply auto 
          ...
          using P_namify_P_name apply simp apply(rule_tac Pn_auto_value) using piauto P_namify_value_in_P_names by auto 
    qed 
  qed
  ...
  then show ?case 
    ...
    apply (rule_tac Forces_Forall) apply simp apply simp apply simp 
    apply (rule_tac Q="(\<forall>x\<in>M. \<pi>`p \<tturnstile> \<phi> map(\<lambda>x. Pn_auto(\<pi>) ` x, map(P_namify, [x] @ env)))" in iff_trans) 
    apply (rule_tac forall_lemma) apply(rule_tac P="arity(\<phi>) \<le> length([x] @ env)" in mp) apply simp
    apply (rule_tac n="arity(\<phi>)" in natE) using arity_type apply simp apply simp 
    using succ_pred_eq apply simp
    apply (rule_tac Q="(\<forall>x\<in>M. \<pi> ` p \<tturnstile> \<phi> map(\<lambda>x. Pn_auto(\<pi>) ` x, [P_namify(x)] @ map(P_namify, env)))" in iff_trans) apply simp
    apply (rule_tac Q="(\<forall>x\<in>M. \<pi> ` p \<tturnstile> \<phi> [Pn_auto(\<pi>) ` P_namify(x)] @ map(\<lambda>x. Pn_auto(\<pi>) ` x, map(P_namify, env)))" in iff_trans) apply simp
    apply (rule_tac Q="(\<forall>x\<in>M. \<pi> ` p \<tturnstile> \<phi> [P_namify(x)] @ map(\<lambda>x. Pn_auto(\<pi>) ` x, map(P_namify, env)))" in iff_trans)
    apply(rule iff_flip) apply (rule_tac iff_lemma) 
    ...
    apply (rule_tac A=M in map_app_distrib) apply simp  
    ...
    apply(rule_tac eq_lemma) apply simp apply simp apply simp 
    apply(rule_tac Q="(\<forall>x\<in>M. \<pi> ` p \<tturnstile> \<phi> [x] @ map(\<lambda>x. Pn_auto(\<pi>) ` x, map(P_namify, env)))" in iff_trans)
    apply(rule iff_flip) apply(rule_tac forall_lemma) apply(rule_tac forces_P_namify) 
    using P_auto_value piauto apply simp apply(rule_tac A=M in app_type) apply simp 
    apply(rule_tac A=M in map_type) apply(rule_tac A=M in map_type) apply simp 
    using P_namify_value_in_M apply simp apply(rule_tac P="xa \<in> P_names"and Q="xa \<notin> P_names" in disjE) 
    apply simp using Pn_auto_value_in_M piauto apply simp using Pn_auto_domain apply_0 zero_in_M apply simp 
    apply simp apply(rule_tac P="arity(\<phi>) = 0"and Q="arity(\<phi>) \<noteq> 0" in disjE) apply blast apply simp 
    apply (rule_tac P="succ(Arith.pred(arity(\<phi>))) \<le> succ(length(env))" in mp) 
    using succ_pred_eq apply simp apply simp apply simp apply(rule iff_flip) apply (rule_tac Forces_Forall) 
    using P_auto_value piauto apply simp apply(rule_tac A=M in map_type) apply(rule_tac A=M in map_type)
    apply simp using P_namify_value_in_M apply simp apply(rule_tac P="x \<in> P_names"and Q="x \<notin> P_names" in disjE) 
    apply simp using Pn_auto_value_in_M piauto apply simp using Pn_auto_domain apply_0 zero_in_M apply simp by simp     
qed 
...
next
  case (Cons a l)
  then show ?case apply simp using P_names_in_M by auto
qed
...
next
  case (Cons a l)
  then show ?case apply auto using P_namify_P_name by auto
qed
...
theorem symmetry_lemma:
  ...
  apply (rule_tac symmetry_lemma') using P_name_list_is_M_list assms apply simp_all 
  apply (rule_tac b="map(P_namify, env)" and a=env in ssubst) 
  using P_namify_P_name_list apply auto done 
